...
class ACO_MinMax(ACO):
    ...
    def __init__(self, t_max=5, **args):
        super().__init__(**args)
        self.k_ = ((self.n_ants - 2) * (0.05 ** (1/float(self.n_ants)))
                   ) / 2*(1 - (0.05 ** (1/float(self.n_ants))))
...
if __name__ == "__main__":
    ...
    aco = ACO_MinMax(t_max=5, graph=g, cars_limit=5,
                     capacity_limit=g.capacity_limit, distance_limit=1000,
                     iterations=1000)
    solution = aco.start()
